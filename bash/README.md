...
1. Copy `bash/setup` from this GitHub repo onto the machine you are bootstrapping (e.g., `curl -o /tmp/setup.sh https://raw.githubusercontent.com/alae-touba/scripts/master/bash/setup`), then make it executable (`chmod +x /tmp/setup.sh`).
...
3. After the script finishes, run `~/work/github/scripts/bash/git-local-conf` to set the repo’s local Git user.name/user.email if needed.
4. Source your shell config (`source ~/.bashrc`) or open a new terminal window so the new aliases and PATH take effect.

> `bash/setup` is idempotent: it reuses `~/work/github/scripts` if already cloned, re-runs the aliases installer, and can be run again later to refresh tooling or fix anything that drifts.
...
`bash/setup` is the bootstrap entry point: it installs core tooling, NVM/Node.js, SDKMAN/Java, Maven, Starship, workspace folders, and ensures aliases are installed.

`bash/bin` contains lightweight helpers you can rely on anywhere:

- `bash/git-local-conf`: sets per-repo Git user.name/user.email and shows the resulting local config.
